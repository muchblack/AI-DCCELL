---
name: ask
description: Async via ask, end turn immediately; use when user explicitly delegates to any AI provider (gemini/opencode/droid); NOT for questions about the providers themselves.
metadata:
...
When `/tmp/.ccb-headless` exists, gemini uses CLI subprocess (`gemini -p`) instead of tmux pane.
...
```
if [ -f /tmp/.ccb-headless ] && [ "$PROVIDER" = "gemini" ]
→ Headless path
else
→ Daemon path
```

**Headless path** (gemini only):
