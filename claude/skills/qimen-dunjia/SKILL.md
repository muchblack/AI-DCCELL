...
3. 你人在哪个城市？如果不在台湾或中国大陆，请直接说国家/城市。
...
本环境已将 `lunar_python` 与 `tzdata` 安装至 `~/.claude/skills/qimen-dunjia/.venv/`。若首次安装或重装，执行：
...
必须使用 venv 内的 python 以避免 macOS PEP 668 限制与依赖缺失：
