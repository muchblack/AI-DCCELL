...
# MLX Code: Delegate Code Writing + Auto Review

Delegate code writing to local MLX running on this Mac,
then perform Linus-style three-tier quality review on the generated code.

## Usage

```
/mlx-code [language] task description
```
...
- `status: "down"` or `"no_model"` → inform user, Claude writes code directly (skip MLX)
...
Present the MLX response to the user:

```
## MLX Generated Result

**Model**: local MLX | **Response time**: {durationMs}ms
...
**Passable (minor fixes)** -> Claude fixes directly (do NOT send back to MLX).
...
- After MLX returns v2, repeat Step 2-3 (display + review)
...
```bash
bash ~/.claude/skills/scripts/telemetry.sh record mlx-code mlx <duration_ms> <result> [--fallback]
```

- `duration_ms`: MLX inference time from MCP response
- `result`: `ok` (🟢) / `passable` (🟡) / `garbage` (🔴) / `error` / `timeout`
- `--fallback`: add if Claude took over due to MLX being unreachable

## Error Handling

- **MLX unreachable**: inform user that local MLX (localhost:8090) is not reachable, suggest checking MLX server
- **Timeout (>3 min)**: inform user inference timed out, suggest shorter or split tasks
...
2. **Analyze failure pattern**: Compare MLX's output vs Claude's correct version. Identify systematic errors:
   - Missing imports, wrong naming conventions, ignored edge cases, poor structure choices, etc.
3. **Edit** the `mlx_code.template` field in `prompts.json` (provider-specific key, won't affect Ollama):
...
- MLX runs locally on Mac; inference speed depends on Apple Silicon performance
